...
CEE, this is a ChemDesk calibration task for the lecture defined in the Lecture metadata section below.
...
Match all attached files to the lecture defined in the Lecture metadata block.
...
Lecture metadata
Update only this block for each lecture.

lecture_id: [enter full stable lecture_id]
lecture_number: [enter lecture number]
lecture_title: [enter lecture title]
chapter_id: [enter chapter_id]
domain_id: [enter domain_id]
project: ChemDesk
source family: [enter source family / lecture source]
...
Use the current lecture_id from the Lecture metadata block.
...
If lecture-specific starting section IDs are supplied in the Lecture metadata block or task instructions, preserve them unless there is a strong reason to adjust.
...
* Use the chapter_id and domain_id supplied in the Lecture metadata block. Do not change them silently.
